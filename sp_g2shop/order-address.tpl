...
<div class="addresses clearfix">
	...
	<p class="address_add submit">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="button button-small btn btn-default">
			<span>{l s='Add a new address'}<i class="fa fa-chevron-right right"></i></span>
		</a>
	</p>
	{if !$opc}
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="pull-left button-exclusive btn btn-default">
					<i class="fa fa-chevron-left"></i>
					{l s='Continue Shopping'}
				</a>
				<button type="submit" name="processAddress" class="button pull-right ">
					<span>{l s='Proceed to checkout'}<i class="fa fa-chevron-right right"></i></span>
				</button>
			</p>
		</form>
{else}
