...
<div itemscope itemtype="http://schema.org/Product">
		...
		<div class="pb-left-column col-xs-12 col-sm-5">
			...
			<div id="image-block" class="clearfix">
				...
				{elseif $product->specificPrice && $product->specificPrice.reduction && $productPriceWithoutReduction > $productPrice}
					<span class="sale-box reduced_price">{l s='Reduced price!'}</span>
				{/if}
				
					
					<div class="price-percent-reduction">
						<span class="txt-label">{l s='Save'} </span>
						<span class="n-sale">{if $product->specificPrice && $product->specificPrice.reduction_type == 'percentage'}{$product->specificPrice.reduction*100}%{/if}</span>
					</div>
				...
				<a href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default')|escape:'html':'UTF-8'}" data-rel="prettyPhoto" class="zoomImg"><i class="fa fa-search"></i></a>
			</div> <!-- end image-block -->
		...
		<div class="pb-right-column  col-xs-12 col-sm-7">
			...
			<ul class="product_reference">
				{if $product->manufacturer_name != ''}
					<li>
				Producent: <a href="index.php?id_manufacturer={$product->id_manufacturer}&controller=manufacturer">
	...
	<div class="moreinfo_block" class="clear">
		<ul class="nav nav-tabs tab-info page-product-heading">
			{if $product->description}<li class="active"><a data-toggle="tab" href="#idTab1">{l s='More info'}</a></li>{/if}
			{if $features}<li ><a data-toggle="tab" href="#idTab2">{l s='Data sheet'}</a></li>{/if}
			<li class=""><a data-toggle="tab" href="#idTab3" >{l s='Reviews'}</a></li>
			{if $accessories}<li ><a data-toggle="tab" href="#idTab4">{l s='Accessories'}</a></li>{/if}
			<!--{if isset($attachments) && $attachments}<li><a href="#idTab6">{l s='Download'}</a></li>{/if}-->
			{if isset($product) && $product->customizable}<li><a href="#idTab6">{l s='Product customization'}</a></li>{/if}	
			{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}<li><a href="#idTab7">{l s='Discount'}</a></li>{/if}	
