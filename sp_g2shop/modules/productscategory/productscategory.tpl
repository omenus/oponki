...
<section class="blockproductscategory">
	<h3 class="productscategory_h3 ">{l s='Related Products' mod='productscategory'}</h3>
		 ...
		 {foreach from=$categoryProducts item='categoryProduct' name=categoryProduct}
			<li class="product-box item">
				<a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" class="lnk_img product-image" title="{$categoryProduct.name|htmlspecialchars}">
					<img class="img_1" src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image, 'home_default')|escape:'html':'UTF-8'}" alt="{$categoryProduct.name|htmlspecialchars}" />
					{if isset($categoryProduct._images.1)}
						<img  class="img_0" src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct._images.1, 'home_default')}"  alt="{$categoryProduct.name|escape:html:'UTF-8'}" />
					{/if}
				</a>
