...
<div id="mywishlist">
    {capture name=path}
        ...
        <a href="{$link->getModuleLink('blockwishlist', 'mywishlist')|escape:'html'}">{l s='Wishlist' mod='blockwishlist'}</a>
		...
		<form method="post" class="std" id="form_wishlist">
			<fieldset>
				...
				<p class="text">
					...
					<input type="text" size="50" id="name" name="name" class="inputTxt" value="{if isset($smarty.post.name) and $errors|@count > 0}{$smarty.post.name|escape:'html':'UTF-8'}{/if}" />
				</p>
				<p class="submit">
			...
			<table class="std">
				...
				{section name=i loop=$wishlists}
					<tr id="wishlist_{$wishlists[i].id_wishlist|intval}">
						...
						<td><a class="button" href="javascript:;" onclick="javascript:WishlistManage('block-order-detail', '{$wishlists[i].id_wishlist|intval}');">{l s='View' mod='blockwishlist'}</a></td>
						<td class=" wishlist_delete">
							<a class="button" href="javascript:;"onclick="return (WishlistDelete('wishlist_{$wishlists[i].id_wishlist|intval}', '{$wishlists[i].id_wishlist|intval}', '{l s='Do you really want to delete this wishlist ?' mod='blockwishlist' js=1}'));">{l s='Delete' mod='blockwishlist'}</a>
						</td>
					</tr>
				{/section}
				</tbody>
	...
	<ul class="footer_links">
		<li><a class="button" href="{$link->getPageLink('my-account', true)|escape:'html'}"><i class="fa fa-user"></i>{l s='Back to Your Account' mod='blockwishlist'}</a></li>
		<li class="f_right"><a class="button" href="{$base_dir}"> <i class="fa fa-home"></i> {l s='Home' mod='blockwishlist'}</a></li>
	</ul>
</div>
