...
<div id="layered_block_left" class="block">
	...
	<div class="block_content">
		<form action="#" id="layered_form" method="post">
			<div>
				{capture name=enabled_filters assign=enabledFilters}
				{if isset($selected_filters) && $n_filters > 0}
				<div id="enabled_filters">
					<span class="layered_subtitle hide" style="float: none;">
						{l s='Enabled filters:' mod='blocklayered'}
					</span>
					<ul>
						{assign var=newfilter value=''}
						{foreach from=$selected_filters key=filter_type item=filter_values}
							{foreach from=$filter_values key=filter_key item=filter_value name=f_values}
								{foreach from=$filters item=filter key=k}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li><a href="#" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>{if $newfilter!=$filter.name}{l s='%1$s: %2$s' mod='blocklayered' sprintf=[$filter.name, $value.name]}{assign var=newfilter value=$filter.name}{else}{l s='%1$s' mod='blocklayered' sprintf=[$value.name]}{/if}</li>
												{/if}
											{/foreach}
										{/if}
									{/if}
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{else}
							<div class="layered_filter col-lg-4 col-md-4 col-sm-6 filter_{$filter.type} {if $filter.type=='manufacturer' || $filter.id_key == 16 || $filter.id_key==17} checker_feature{/if}">
						{/if}
                        <div class="layered_subtitle_heading">
							...
							{if $filter.type=='manufacturer'}
								...
								<ul class="col-lg-12 layered_filter_ul">
									...
									<select class="select form-control" style="pointer-events: none">
											<option value="">{l s='Wybierz' mod='blocklayered'}</option>
									</select>
								
								</ul>
						    {/if}
							
							{if $filter.type=='id_feature' and ($filter.id_key == 16 || $filter.id_key == 17 )}
						    ...
						    {/if}
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if $filter.type=='manufacturer' || $filter.id_key == 16 || $filter.id_key==17} filter_checker hide{/if}">
							
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{if $filter.type=='manufacturer'}
									{function name=manufactorergroup group=1}
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
											{if $value.type==$group}
											<li class="group_{$value.type} nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
									...
									{/foreach}
									{/function}
									<li class="group_all"><input type="checkbox" name="checkall" id="ca_{$filter.type}_{$filter.id_key}"><label for="ca_{$filter.type}_{$filter.id_key}"> Zaznacz wszystkie </label></li>
									<li class="group group_head"><span class="group-title"><input type="checkbox" class="checkgroup" id="cag1_{$filter.type}_{$filter.id_key}"/> <label for="cag1_{$filter.type}_{$filter.id_key}">Klasa budżet</label></span><ul class="group-list">{call name=manufactorergroup group=1}</ul>
									</li>
									<li class="group group_head"><span class="group-title"><input type="checkbox" class="checkgroup" id="cag2_{$filter.type}_{$filter.id_key}"/> <label for="cag2_{$filter.type}_{$filter.id_key}">Klasa średnia</label></span><ul class="group-list">{call name=manufactorergroup group=2}</ul>
									</li>
									<li class="group group_head"><span class="group-title"><input type="checkbox" class="checkgroup" id="cag3_{$filter.type}_{$filter.id_key}"/> <label for="cag3_{$filter.type}_{$filter.id_key}">Klasa premium</label></span><ul class="group-list">{call name=manufactorergroup group=3}</ul>
									</li>
									<li class="group group_head"><span class="group-title"><input type="checkbox" class="checkgroup" id="cag4_{$filter.type}_{$filter.id_key}"/> <label for="cag4_{$filter.type}_{$filter.id_key}">Inne</label></span><ul class="group-list">{call name=manufactorergroup group=4}</ul>
									</li>
									{elseif $filter.type == 'id_feature' && ($filter.id_key == 16 || $filter.id_key==17)}
										
										{function name=feature group=1}
											<li class="group_all"> <input type="checkbox" name="checkall" id="caz_{$filter.type_lite}_{$filter.id_key}"> <label for="caz_{$filter.type_lite}_{$filter.id_key}">Zaznacz wszystkie </label></li>
											<ul class="group-list">
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
											
											<li class="group_feature nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if}">
												<input type="checkbox" class="checkbox" name="layered_{$filter.type_lite}_{$id_value}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}{if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}{if !$value.nbr} disabled="disabled"{/if} /> 
												<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
									...
									{/function}
									<li class="group_check"><span class="group-title"></span><ul class="group">{call name=feature group=1}</ul>
									</li>
			...
			{foreach from=$filters item=filter}
				...
				{/if}
			{/foreach}
			<button type="submit" class="submit-button button right cleafix bottom-line">
				<span>Szukaj<i class="fa fa-chevron-right right"></i></span>
			</button>
