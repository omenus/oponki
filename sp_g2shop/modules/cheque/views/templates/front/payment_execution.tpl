	...
	<form action="{$link->getModuleLink('cheque', 'validation', [], true)|escape:'html':'UTF-8'}" method="post">
		...
		<p class="cart_navigation clearfix" id="cart_navigation">
			<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html':'UTF-8'}" class="button-exclusive btn btn-default">
				<i class="fa fa-chevron-left left"></i>{l s='Other payment methods' mod='cheque'}
			</a>
			<button type="submit" class="button btn btn-default button-medium pull-right">
				<span>{l s='I confirm my order' mod='cheque'}<i class="fa fa-chevron-right right"></i></span>
			</button>
		</p>
	</form>
{/if}
